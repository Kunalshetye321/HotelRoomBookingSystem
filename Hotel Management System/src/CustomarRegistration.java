...
public class CustomarRegistration extends JFrame implements ActionListener
{
	private JPanel panel;
	private JLabel labelCustomarID,labelDisplayCustomarID,labelFirstName, labelLastName,labelEmail,labelMobileNumber,labelGender,lebelMaritorialStatus,labelNationalID,labelAddress,labelPassportNumber,labelNumberOfAdults,labelNumberOfChildrens;
	...
	private JComboBox comboboxChildrens,comboboxAdults;
	...
	private void InitializeComponents()
	{
		...
		for(int i=0;i<data.getCustomarID().length;i=i+2)
		{
			this.CustomarId = Integer.parseInt((String)data.getCustomarID()[i]);
			int j = Integer.parseInt((String)data.getCustomarID()[i+1]);
			if(this.CustomarId < j)
			{
				this.CustomarId = j;
			}
		}
		...
		this.labelDisplayCustomarID = new JLabel((CustomarId+1)+"");
		this.labelDisplayCustomarID.setBounds(200, 20, 100, 20);
		this.panel.add(this.labelDisplayCustomarID);
		...
		this.buttonUpdate.setBounds(250, 450, 80, 20);
	...
	public void actionPerformed(ActionEvent ae) 
	{
		...
		{
			UpdateCustomarInfo  update = new UpdateCustomarInfo();
			update.setVisible(true);
			this.setVisible(false);
		}
		else if(ae.getActionCommand() == "Save")
		{
			int CustomarID = Integer.parseInt((String)this.labelDisplayCustomarID.getText());
			...
			customarData.setCustomarID(CustomarID);
			
			if(data.InsertCustomar(customarData) == true)
			{
